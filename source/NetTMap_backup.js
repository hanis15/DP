...
// ------------------------------------------------------------------------------------------------------------
    ...
    function load_local_variable() {
        // nacteni z HTML5 lokalniho uloziste
        if (typeof (Storage) !== "undefined") {
            if (sessionStorage.nodes != null) nodes = jQuery.parseJSON(sessionStorage.nodes);
            if (sessionStorage.links != null) links = jQuery.parseJSON(sessionStorage.links);
            if (sessionStorage.current_item_index != null) current_item_index = sessionStorage.current_item_index;
            if (sessionStorage.current_type != null) current_type = sessionStorage.current_type;
        } else {
            alert("Local storage isn't support");
        }
    }
